...
const DEFAULT_IMAGE_DURATION = 5000; // Keep normal images pacing at 5 seconds
...
function plusSlides(n) {
  clearTimeout(slideshowTimeout); // Halt auto timeline when user interacts manually
...
function showSlides(n) {
  ...
  // Reset display status and pause background assets safely
  for (i = 0; i < slides.length; i++) {
    ...
    if (video) {
      video.pause();
      video.currentTime = 0; // Rewind background clips to start
      video.onended = null;  // Clear active handlers so they don't loop/stack up
    }
  }
  ...
  // Determine active slide asset type
  let activeVideo = slides[slideIndex - 1].querySelector("video");
  
  if (activeVideo) {
    // SCENARIO A: Target slide is a video file
    activeVideo.play().then(() => {
      // Direct hook: Trigger the next slide immediately once video naturally finishes (1:26)
      activeVideo.onended = function() {
        plusSlides(1);
      };
    }).catch(error => {
      console.log("Autoplay prevented by browser context. Falling back to default timing limit.");
      // Safety execution if security permissions block execution
      slideshowTimeout = setTimeout(() => { plusSlides(1); }, DEFAULT_IMAGE_DURATION);
    });
    
  } else {
    // SCENARIO B: Target slide is an image file (Keep original 5s pacing)
