...
/**
 * DYNAMIC VIDEO & IMAGE SLIDER
 */

let slideIndex = 1;
let sliderTimeout;
const VIDEO_DURATION_MS = (26 * 60 + 58) * 1000; // 26:58 in milliseconds (1618000ms)
...
function showSlides(n) {
  ...
  // Handle Video element state on slide change
  const video = document.getElementById("sculptureVideo");
  if (video) {
    if (slideIndex === 1) {
      video.currentTime = 0;
...
function runDynamicSequence() {
  clearTimeout(sliderTimeout);
  
  if (slideIndex === 1) {
    // Slide 1: Wait for the full video duration (9m 46s)
    ...
    // Slide 2: Wait 8 seconds for the static image
    sliderTimeout = setTimeout(() => {
      slideIndex = 1;
      showSlides(slideIndex);
      runDynamicSequence();
...
// Hook into the HTML video element's 'ended' event as a fallback/failsafe
